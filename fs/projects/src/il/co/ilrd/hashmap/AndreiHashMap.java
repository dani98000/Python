 ...
 * @param <V> - value type
 */
public class AndreiHashMap<K, V> implements Map<K, V> {
    private static final int DEFAULT_CAPACITY = 16;
    private List<Entry<K, V>>[] buckets;
    volatile int modCountEntries;

    public AndreiHashMap() {
	this(DEFAULT_CAPACITY);
    }

    public AndreiHashMap(int initialCapacity) {
	...
	buckets[key.hashCode() % buckets.length].add(Pair.of(key, value));
	...
	// Set<Entry<? extends K, ? extends V> > sourceEntries =
	// sourceMap.entrySet();//how to do it?
	...
	final List<Entry<K, V>> bucket = buckets[bucketIndex];
	...
	for (Entry<K, V> entry : buckets[key.hashCode() % buckets.length]) {
